make_est_d = function(fits, X, d) {
    ...
    cli::cli_process_start("BIRDiE fits")
    d_birdie = map(fits, function(ll) {
        map(ll, function(res) {
            out = tidy(res)
            if (!is.null(res$se)) out$se = c(res$se)
            out
        }) |>
            bind_rows(.id = "method")
    }) |>
    ...
    cli::cli_process_start("Weighting estimator")
    form = as.formula(paste(X_name, "~ 1"))
    d_wtd = map(r_probs, function(d_pr) {
        est = est_weighted(d_pr, form, data=d, se_boot=100)
        out = tidy(est)
        out$se = c(est$se)
        out
    }) |>
    ...
    cli::cli_process_start("Thresholding estimator")
    d_thresh = map(r_probs, function(d_pr) {
        tidy_thresh(d_pr, X, X_name, se_boot=100)
    }) |>
        bind_rows(.id = "level") |>
        mutate(method = "thresh")
    cli::cli_process_done()

    cli::cli_process_start("OLS estimator")
    d_ols = imap(r_probs, function(d_pr, level) {
        if (level == "block") level = "tract"
        level = str_c("GEOID_", level)
        tidy_ols(d_pr, X, d[[level]], X_name) |>
            suppressWarnings() # singular fits
    }) |>
        bind_rows(.id = "level") |>
        mutate(method = "ols")
    cli::cli_process_done()

    bind_rows(d_birdie, d_wtd, d_thresh, d_ols) |>
        rename(est = estimate) |>
...
calc_disp <- function(ests) {
    ests |>
        ...
        pivot_wider(names_from=race, values_from=c(est, est_true, se)) |>
        mutate(disp_wb = est_white - est_black,
               disp_wb_se = sqrt(se_white^2 + se_black^2),
               disp_wb_true = est_true_white - est_true_black,
               disp_wh = est_white - est_hisp,
               disp_wh_se = sqrt(se_white^2 + se_hisp^2),
               disp_wh_true = est_true_white - est_true_hisp) |>
        select(-starts_with("est_"), -starts_with("se_"))
}

eval_fit_tv = function(ests, p_r) {
    N = 1000
    rnm = function(n, est, se) matrix(rnorm(N * n(), mean=rep(est, N), sd=rep(se, N)), ncol=N)

    tv_overall = ests |>
        group_by(level, method) |>
        summarize(tv = sum(abs(est - est_true)*p_r[race])/2,
                  tv_se = sd(colSums(abs(rnm(n(), est, se) - est_true) * c(p_r[race])))) |>
        suppressWarnings() |>
        mutate(race = "overall")
...
filter(disp_party, level=="county", party=="dem") |>
    select(-level, -party) |>
    split(~ method) |>
...
methods = c(birdie_pool="BIRDiE (pooling)", birdie_sat="BIRDiE (saturated)",
            birdie_sat_gibbs="BIRDiE (Gibbs)", birdie_mmm="BIRDiE (mixed)",
            ols="OLS", thresh="Threshold", weight="Weighting")
methods_short = c(birdie_pool="BIRDiE (pool.)", birdie_sat="BIRDiE (sat.)",
                  birdie_sat_gibbs="BIRDiE (Gibbs)", birdie_mmm="BIRDiE (mixed)",
                  ols="OLS", thresh="Threshold", weight="Weighting")
methods_col = c(birdie_pool=PAL_R[3], birdie_sat=PAL_R[3],
                birdie_sat_gibbs=PAL_R[3], birdie_mmm=PAL_R[3],
                ols=PAL_R[2], thresh=PAL_R[4], weight=PAL_R[1])
methods_shp = c(birdie_pool=16, birdie_sat=15, birdie_sat_gibbs=17, birdie_mmm=16,
                ols=5, thresh=4, weight=1)
...
filter(disp_party, level == "county") |>
...
ggplot(aes(x=factor(geos[level], levels=geos), y=tv,
    ...
    geom_textline(aes(label=methods_short[method],
                      hjust=c(birdie_sat=0.05, birdie_pool=0.08, birdie_mmm=0.50,
                              birdie_sat_gibbs=0.7, ols=0.08, weight=0.08, thresh=0.08)[method]),
                  position=position_dodge(width=0.7),
                  linewidth=0.7, size=3.0, family="Times") +
    geom_linerange(aes(ymin = tv - 1.96*tv_se, ymax = tv + 1.96*tv_se),
                   position=position_dodge(width=0.7)) +
    geom_point(size=2.4, position=position_dodge(width=0.7)) +
...
d_small = map_dfr(geo_levels[1:3], function(l) {
    ...
    bind_rows(
        ...
        thresh = map(idxs, ~ tidy_thresh(r_probs[[l]][., ], d$party[.], "party")) |>
            bind_rows(.id="GEOID"),
        ols = map(idxs, ~ tidy_ols(r_probs[[l]][., ], d$party[.], rep_along(., 1), "party")) |>
            bind_rows(.id="GEOID"),
        .id="method"
    ) |>
        mutate(level = l, .before=everything())
